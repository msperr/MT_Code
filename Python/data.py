...
if __name__ == '__main__':
    ...
    if split:
        ...
        if args.export:
            xpressfile = '%s.split%d.txt' % (basename, len(split)+1)
            ...
            filename = '%s.split%d.graph.json' % (basename, len(split)+1)
            taskgraph.save_taskgraph_to_json(G, filename)
